...
| 変数 | デフォルト | 説明 |
|---|---|---|
| `FRONTEND_ORIGIN` | 空 | フロントエンドアドレス。招待メールのリンク、CORS 許可リスト、cookie ドメインはすべてこの値から派生します。設定しない場合、招待メールのリンクはホスト型ドメイン `https://multica.ai` にフォールバックします — セルフホストはこれを明示的に設定する必要があります |
...
| `GITHUB_APP_ID` | 空 | PR 連携だけなら任意。リポジトリベースのエージェントには必須 |
| `GITHUB_APP_PRIVATE_KEY` | 空 | PR 連携だけなら任意。リポジトリベースのエージェントには必須。完全な RSA 秘密鍵 PEM ブロック |
| `GITHUB_API_BASE_URL` | `https://api.github.com` | GitHub Enterprise またはテストプロキシ用の任意の API ベース URL |
...
- PR 連携は動作しますが、GitHub リポジトリからのエージェント作成と同期は利用できません。
