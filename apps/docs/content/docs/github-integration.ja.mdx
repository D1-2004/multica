...
ルートの `multica-agent.yaml` で `apiVersion: multica.ai/v1alpha1`、`kind: Agent`、名前、説明、指示ファイル、スキルディレクトリ、任意の対応 provider を宣言します。各スキルディレクトリには `SKILL.md` と任意の補助ファイルを置きます。

Multica は選択した commit を検証し、データベースのスナップショットとして保存します。名前、説明、指示、リポジトリのスキルは GitHub 管理、ランタイム、モデル、アクセス、追加スキルは Multica 管理です。変更は自動反映されず、エージェント Overview の **今すぐ同期**で明示的に同期します。
...
installation と pull request の連携テーブルはマイグレーション `079_github_integration`、GitHub エージェントソースのテーブルは `167_github_agent_source` で提供されます。古いデプロイをアップグレードする場合:
