...
ルートには `dingtalk-agent/project@1` の DTA `dingtalk-agent.json` が必要です。`agent.definition` は定義ファイルを指し、`agent.skills` は `dingtalk-basic-behavior` を明示的に含めます。Basic は `.agents/skills/dingtalk-basic-behavior`、その他のスキルは `skills/<name>` に配置し、各 `SKILL.md` の frontmatter `name` を宣言名と一致させます。
