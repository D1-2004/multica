...
ルートには `dingtalk-agent/project@1` の DTA `dingtalk-agent.json` が必要です。`agent.definition` は配信する定義ファイルを指し、`agent.skillsRoot` は必須で、`agent.skills` は `dta-basic-behavior` を明示的に含めます。Basic を含むすべてのスキルは `<skillsRoot>/<name>` に配置し、各 `SKILL.md` の frontmatter `name` を宣言名と一致させます。ルートの `AGENTS.md` はリポジトリ開発ルールに利用でき、インポートされるのは `agent.definition` が指すファイルだけです。
