...
Multica は `project@1` の安定フィールドを検証し、追加の DTA 拡張フィールドと `workspaces` の内部定義は解釈しません。選択した commit の定義とリポジトリスキルをスナップショット化しますが、作成後に Multica で編集した名前と説明は同期時も保持します。従来の `multica-agent.yaml` にはフォールバックしません。変更は自動反映されず、エージェント Overview の **今すぐ同期**で明示的に同期します。
