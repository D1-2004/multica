...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `FRONTEND_ORIGIN` | 空 | 前端地址。邀请邮件里的链接、CORS 白名单、cookie domain 都从这里推导。邮件链接在不设时会 fallback 到托管版域名 `https://multica.ai`——self-host 必须显式填 |
...
<Callout type="warning">
**`FRONTEND_ORIGIN` 不设就有两个静默失败**：（1）邀请邮件里的链接指向 `https://multica.ai`（托管版的域名），用户点了跳不回你的 self-host 实例；（2）WebSocket 连接的 Origin 校验回落到 `localhost:3000 / 5173 / 5174`，生产部署的 WebSocket 全部被拒，前端看起来「实时更新不工作」。
</Callout>
...
| `GITHUB_APP_ID` | 空 | 仅做 PR 关联时可选；仓库来源智能体必填。配合 `GITHUB_APP_PRIVATE_KEY` 签发 installation token |
| `GITHUB_APP_PRIVATE_KEY` | 空 | 仅做 PR 关联时可选；仓库来源智能体必填。App RSA 私钥的完整 PEM 块（包含 `-----BEGIN/END-----` 两行，保留换行）|
| `GITHUB_API_BASE_URL` | `https://api.github.com` | 可选的 GitHub 兼容 API 地址，主要用于 GitHub Enterprise 或测试代理 |
...
- PR 关联仍可使用，但无法从 GitHub 仓库创建或同步智能体。
