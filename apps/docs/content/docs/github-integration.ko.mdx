...
GitHub App에 **Contents: Read-only** 권한이 있고 App 자격 증명이 설정되어 있으면 **Agents → 에이전트 만들기 → GitHub에서 가져오기**를 사용할 수 있습니다. 저장소는 정의의 버전 관리 소스이며 런타임 작업 디렉터리가 아닙니다.

루트에는 `dingtalk-agent/project@1` 형식의 DTA `dingtalk-agent.json`이 필요합니다. `agent.definition`은 배포할 정의 파일을 가리키고 `agent.skillsRoot`는 필수이며 `agent.skills`에는 `dingtalk-basic-behavior`를 명시적으로 포함해야 합니다. Basic을 포함한 모든 스킬은 `<skillsRoot>/<name>`에 두고 각 `SKILL.md`의 frontmatter `name`은 선언한 이름과 일치해야 합니다. 루트 `AGENTS.md`는 저장소 개발 규칙에 사용할 수 있으며 `agent.definition`이 가리키는 파일만 가져옵니다.

Multica는 `project@1`의 안정 필드를 검증하고 추가 DTA 확장 필드와 `workspaces` 내부 정의는 해석하지 않습니다. display name은 이름의 생성 기본값입니다. `project@1`에는 별도의 에이전트 설명 필드가 없으므로 처음 선언된 Basic 이외의 Role Skill description을 설명의 생성 기본값으로 사용합니다. 저장소 스킬은 생성 폼에 선택된 상태로 바로 표시되며 일반 워크스페이스 스킬 선택이 아니라 GitHub Source가 자동으로 구체화합니다. 생성 후 Multica에서 편집한 이름과 설명은 동기화할 때 유지합니다. 기존 `multica-agent.yaml`로는 폴백하지 않습니다. 변경 사항은 자동 적용되지 않으며 에이전트 Overview의 **지금 동기화**로 명시적으로 동기화합니다.
...
- GitHub App은 pull request, 메타데이터 및 저장소 Contents에 대한 **읽기 전용** 액세스를 요청합니다. Multica는 GitHub에 다시 쓰지 않습니다.
...
| **Permissions → Repository → Contents** | **Read-only**. 저장소에서 에이전트를 만들고 동기화하는 데 필요합니다. |
...
# PR 연동만 사용할 때는 선택, GitHub 에이전트 생성과 동기화에는 필수:
...
`GITHUB_APP_ID`와 `GITHUB_APP_PRIVATE_KEY`는 PR 연동만 사용할 때는 선택 사항이지만 저장소 기반 에이전트 생성과 동기화에는 둘 다 필요합니다. Multica는 이를 사용해 단명 installation access token을 발급하며 token은 데이터베이스에 저장하지 않습니다.
...
installation 및 pull request 연동 테이블은 마이그레이션 `079_github_integration`으로, GitHub 에이전트 소스 테이블은 `169_github_agent_source`로 제공됩니다. 기존 배포를 업그레이드하는 경우:
