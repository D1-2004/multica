...
루트에는 `dingtalk-agent/project@1` 형식의 DTA `dingtalk-agent.json`이 필요합니다. `agent.definition`은 정의 파일을 가리키고 `agent.skills`에는 `dingtalk-basic-behavior`를 명시적으로 포함해야 합니다. Basic은 `.agents/skills/dingtalk-basic-behavior`, 그 외 스킬은 `skills/<name>`에 두며 각 `SKILL.md`의 frontmatter `name`은 선언한 이름과 일치해야 합니다.
