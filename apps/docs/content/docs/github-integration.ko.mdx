...
Multica는 `project@1`의 안정 필드를 검증하고 추가 DTA 확장 필드와 `workspaces` 내부 정의는 해석하지 않습니다. 선택한 commit의 정의와 저장소 스킬을 스냅샷으로 저장하지만, 생성 후 Multica에서 편집한 이름과 설명은 동기화할 때 유지합니다. 기존 `multica-agent.yaml`로는 폴백하지 않습니다. 변경 사항은 자동 적용되지 않으며 에이전트 Overview의 **지금 동기화**로 명시적으로 동기화합니다.
