...
루트의 `multica-agent.yaml`에서 `apiVersion: multica.ai/v1alpha1`, `kind: Agent`, 이름, 설명, 지침 파일, 스킬 디렉터리 및 선택적 호환 provider를 선언합니다. 각 스킬 디렉터리에는 `SKILL.md`와 선택적 보조 파일을 둡니다.

Multica는 선택한 commit을 검증하고 데이터베이스 스냅샷으로 저장합니다. 이름, 설명, 지침 및 저장소 스킬은 GitHub에서 관리하고 런타임, 모델, 접근 권한 및 추가 스킬은 Multica에서 관리합니다. 변경 사항은 자동 적용되지 않으며 에이전트 Overview의 **지금 동기화**로 명시적으로 동기화합니다.
...
installation 및 pull request 연동 테이블은 마이그레이션 `079_github_integration`으로, GitHub 에이전트 소스 테이블은 `167_github_agent_source`로 제공됩니다. 기존 배포를 업그레이드하는 경우:
