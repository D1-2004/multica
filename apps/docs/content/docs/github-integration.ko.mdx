...
루트에는 `dingtalk-agent/project@1` 형식의 DTA `dingtalk-agent.json`이 필요합니다. `agent.definition`은 배포할 정의 파일을 가리키고 `agent.skillsRoot`는 필수이며 `agent.skills`에는 `dta-basic-behavior`를 명시적으로 포함해야 합니다. Basic을 포함한 모든 스킬은 `<skillsRoot>/<name>`에 두고 각 `SKILL.md`의 frontmatter `name`은 선언한 이름과 일치해야 합니다. 루트 `AGENTS.md`는 저장소 개발 규칙에 사용할 수 있으며 `agent.definition`이 가리키는 파일만 가져옵니다.
