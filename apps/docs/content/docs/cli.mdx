...
| `multica agent template list/get` | Discover approved Git repository agent templates |
| `multica agent create-from-template <key> --runtime-id <id>` | Create an agent from an approved Git template |
...
| `multica dingtalk install begin/status` | Start or inspect a DingTalk bot installation session |

Git agent templates are approved repositories containing
`multica-agent.yaml`. The manifest name and description are creation defaults;
the Agent instance can override and later edit them in Multica. Repository sync
continues to own instructions and repository-managed skills. The
`create-from-template` command does not expose model or thinking-level flags.
