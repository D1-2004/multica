...
The sandbox template must provide `multica`, `hermes`, and `multica-fc-hermes-runner` on `PATH`. An FC agent may pin one model from `MULTICA_FC_E2B_OPENAI_MODELS`; agents without a pinned model use the first item. The runner writes the resolved model into Hermes' own config so Hermes keeps using its custom provider credentials. When a task carries `agent_identity_context_token`, the runner exchanges it once during sandbox startup and DWS refreshes the resulting login with its refresh token for the rest of the task.
