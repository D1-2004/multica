...
The sandbox template must provide `multica`, the agent CLIs it hosts, and one root-owned entry per hosted agent at `/usr/local/libexec/multica-fc-<provider>-container-log-entry`. Multica starts that fixed path as root so it can attach stdout/stderr to the FC container log stream; the entry then permanently drops to uid/gid 1000 before running the shared task runner. Older templates without this entry are not supported. A template may ship several agent CLIs side by side; each runtime's provider (`hermes` or `opencode`) is chosen when the runtime is created — via the `provider` field on `POST /api/runtimes/fc-e2b` or the runtime selector in the creation dialog. A template whose name contains `opencode` merely preselects that provider; everything else defaults to `hermes`. An FC agent may pin one model from `MULTICA_FC_E2B_OPENAI_MODELS`; agents without a pinned model use the first item. The runner writes the resolved model into the agent's own config (for example Hermes' config file) so the agent keeps using its custom provider credentials. When a task carries `agent_identity_context_token`, the runner exchanges it once during sandbox startup and DWS refreshes the resulting login with its refresh token for the rest of the task.
