...
| `MULTICA_FC_E2B_OPENAI_MODEL` | empty | Hermes model written into the sandbox's Hermes config |
...
The sandbox template must provide `multica`, `hermes`, and `multica-fc-hermes-runner` on `PATH`. For Hermes with a custom OpenAI-compatible endpoint, do not set `MULTICA_HERMES_MODEL` on the agent or daemon; the runner writes the model into Hermes' own config so Hermes keeps using its custom provider credentials.
