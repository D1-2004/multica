...
每个 skill path 指向一个包含 `SKILL.md` 和可选配套文件的目录。Multica 校验清单后，把所选 commit 编译为数据库快照。名称、描述、指令和仓库 skill 由 GitHub 管理；runtime、model、访问权限和额外工作区 skill 仍由 Multica 管理。
...
installation 与 pull request 集成表由 migration `079_github_integration` 提供；GitHub 智能体来源表由 `167_github_agent_source` 提供。如果是升级既有部署：
