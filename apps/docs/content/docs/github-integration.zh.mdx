...
公共 `dingtalk-basic-behavior` 位于 `.agents/skills/dingtalk-basic-behavior`，其他声明的 skill 位于 `skills/<name>`。每个目录都包含 `SKILL.md` 和可选配套文件，并且 frontmatter 的 `name` 必须与声明名称一致。
