...
Multica 校验 `project@1` 的稳定字段后，把所选 commit 编译为数据库快照；DTA 新增的扩展字段可以存在，`workspaces` 内部配置不会由 Multica 解释。display name 只作为创建默认值，后续同步更新本体和仓库 skill，并保留在 Multica 中修改的名称和描述。runtime、model、访问权限和额外工作区 skill 也仍由 Multica 管理。GitHub 导入不会回退读取旧的 `multica-agent.yaml`。
