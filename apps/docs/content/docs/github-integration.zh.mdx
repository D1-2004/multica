...
`agent.skillsRoot` 是必填字段。包括 `dta-basic-behavior` 在内的所有声明 skill 都位于 `<skillsRoot>/<name>`；在这个例子中对应 `agent/skills/dta-basic-behavior` 和 `agent/skills/code-review`。每个目录都包含 `SKILL.md` 和可选配套文件，并且 frontmatter 的 `name` 必须与声明名称一致。根目录 `AGENTS.md` 可以保存仓库开发规则，只有 `agent.definition` 指向的文件会作为智能体本体导入。
