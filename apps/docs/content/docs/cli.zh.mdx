...
| `multica agent template list/get` | 查看已批准的 Git 仓库智能体模板 |
| `multica agent create-from-template <key> --runtime-id <id>` | 从已批准的 Git 模板创建智能体 |
...
| `multica dingtalk install begin/status` | 发起或查看钉钉机器人安装会话 |

Git 智能体模板是包含 `multica-agent.yaml` 的已批准仓库。Manifest 中的名称和描述只是创建默认值，智能体实例可在创建时覆盖并在 Multica 中继续修改；仓库同步仍管理 instructions 和仓库内的 skill。`create-from-template` 不提供 model 或 thinking-level 参数。
