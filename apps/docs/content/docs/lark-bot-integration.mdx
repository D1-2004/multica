...
| **Run card for `/issue`** | When the issue's assigned agent picks up the run, the Bot posts a status card bound to that run: live status and progress, an expandable run-detail panel (recent tool calls and agent notes), a **Terminate** button while the run is active, and an **Open in Multica** link. |
