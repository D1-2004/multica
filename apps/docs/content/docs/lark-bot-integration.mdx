...
| **Run card** | Every agent run triggered from Lark — a chat question or a `/issue`-born issue — gets a status card bound to that run: live status and progress, an expandable run-detail panel (recent tool calls and agent notes), and a **Terminate** button while the run is active. Issue runs also carry an **Open in Multica** link; the final answer still arrives as a normal message. |
