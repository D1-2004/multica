...
| `multica agent template list/get` | 승인된 Git 저장소 에이전트 템플릿 조회 |
| `multica agent create-from-template <key> --runtime-id <id>` | 승인된 Git 템플릿에서 에이전트 생성 |
...
| `multica dingtalk install begin/status` | DingTalk 봇 설치 세션 시작 또는 상태 조회 |

Git 에이전트 템플릿은 `multica-agent.yaml`을 포함하는 승인된 저장소입니다. Manifest의 이름과 설명은 생성 기본값이며 Multica에서 덮어쓰고 나중에 편집할 수 있습니다. 저장소 동기화는 지침과 저장소 관리 스킬만 관리합니다. `create-from-template`에는 model 또는 thinking-level 옵션이 없습니다.
