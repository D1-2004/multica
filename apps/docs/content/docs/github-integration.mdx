...
The shared `dingtalk-basic-behavior` skill lives at `.agents/skills/dingtalk-basic-behavior`; each other declared skill lives at `skills/<name>`. Every skill directory contains a `SKILL.md` whose frontmatter `name` matches the declared name, plus optional supporting files.
