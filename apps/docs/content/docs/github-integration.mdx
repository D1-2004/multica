...
The repository root must contain `multica-agent.yaml`:
...
metadata:
  ...
  description: Reviews changes using repository-owned guidance.
spec:
...
Each skill path is a directory containing `SKILL.md` plus optional supporting files. Multica validates the manifest and materializes the selected commit into its database as a snapshot. Name, description, instructions, and repository skills are GitHub-managed; runtime, model, access, and additional workspace skills remain Multica-managed.
