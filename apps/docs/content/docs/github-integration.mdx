...
`agent.skillsRoot` is required. Every declared skill, including `dingtalk-basic-behavior`, lives at `<skillsRoot>/<name>`. In this example the paths are `agent/skills/dingtalk-basic-behavior` and `agent/skills/code-review`. Every skill directory contains a `SKILL.md` whose frontmatter `name` matches the declared name, plus optional supporting files. Root `AGENTS.md` may contain repository development rules; only the file selected by `agent.definition` becomes the imported Agent definition.

Multica validates the stable `project@1` fields and materializes the selected commit into its database as a snapshot. It accepts additional DTA extension fields and does not interpret workspace definitions. The display name is the name creation default. Because `project@1` has no separate Agent description field, Multica uses the first declared non-Basic Role Skill description as the description creation default. Repository skills are shown selected in the creation form and are materialized automatically; they are not ordinary workspace-skill selections. Later syncs update the definition and repository skills while preserving the name and description edited in Multica. Runtime, model, access, and additional workspace skills also remain Multica-managed. GitHub import does not fall back to the legacy `multica-agent.yaml` format.
