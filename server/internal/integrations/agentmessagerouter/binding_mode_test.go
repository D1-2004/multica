...
func TestMessageBindingCallbackAtomicallyStoresIdentityAndRoute(t *testing.T) {
	now := time.Date(2026, 7, 17, 10, 0, 0, 0, time.UTC)
	...
	if !store.activated || store.identity.DwsUid != "24710833" || store.identity.OrgID != "439446171" {
		t.Fatalf("route activated=%v identity=%#v", store.activated, store.identity)
	}
	if binding.DWSIdentity.Status != "active" || binding.DWSIdentity.Source != "message" || binding.MessageRoute.Status != "active" {
	...
	for _, secret := range []string{"24710833", "439446171", canonicalCallbackToken, "source-1"} {
