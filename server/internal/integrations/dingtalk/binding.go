...
// This file is the DingTalk user-binding token flow: an unbound DingTalk
// user who messages the bot gets a "link your account" prompt (minted
// here, delivered by the OutboundReplier through the message's session
// webhook), clicks through to the in-product redeem page (/dingtalk/bind),
// and their DingTalk staff id is bound to their Multica account. It
// mirrors slack.BindingTokenService on the generic channel_* queries with
...
// is transactional: consuming the token and inserting the
// channel_user_binding row commit together, so a failed bind never burns a
// token.
...
// Mint creates a single-use binding token for (installation, dingTalkUserID)
// and returns the raw secret + expiry. The raw value must be delivered over
// DingTalk (encrypted in transit by the platform) and never logged.
...
// RedeemAndBind atomically consumes a raw token and binds the DingTalk user
