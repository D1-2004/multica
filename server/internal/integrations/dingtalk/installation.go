...
var (
	...
	// ErrRouterUnavailable is reserved for HTTP_CALLBACK lifecycle operations.
	// Stream installation and revocation never depend on Router availability.
...
func (s *InstallationService) Upsert(ctx context.Context, p InstallationParams) (Installation, error) {
	...
	cfg, err := encodeInstallConfig(Installation{
		...
		ConnectionManaged:  p.TransportMode != TransportModeHTTPCallback,
		RobotCode:          p.RobotCode,
		DispatchEndpointID: p.DispatchEndpointID,
		RouterSourceID:     p.RouterSourceID,
		RouterStatus:       p.RouterStatus,
		RouterLastError:    p.RouterLastError,
...
func (s *InstallationService) UpdateRouterState(
	ctx context.Context,
	inst Installation,
	sourceID, status, lastError string,
) (Installation, error) {
	if s.queries == nil {
		return Installation{}, errors.New("dingtalk installation store is not configured")
	}
	inst.RouterSourceID = strings.TrimSpace(sourceID)
	inst.RouterStatus = strings.TrimSpace(status)
	inst.RouterLastError = strings.TrimSpace(lastError)
	return s.queries.UpdateDingTalkInstallationConfig(ctx, inst)
}
...
func validateInstallationParams(p InstallationParams) error {
	...
	case strings.TrimSpace(p.RobotCode) == "":
		return errors.New("robot_code is required")
	}
	_, err := normalizeTransportMode(p.TransportMode)
	return err
}
