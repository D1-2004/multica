...
func runDaemonRunOnce(cmd *cobra.Command, _ []string) error {
	...
	logger := logger_pkg.NewLogger("daemon")
	// The daemon-side feature-flag snapshot chain (execenv server snapshot
	// provider) was retired upstream along with featureflagdispatch; flags now
	// flow through the server-evaluated featureflag.Service only.
