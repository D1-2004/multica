...
func TestRunIssueCreateSendsAgentIdentityContextToken(t *testing.T) {
	...
	_ = cmd.Flags().Set("title", "Context token issue")
	_ = cmd.Flags().Set("agent-identity-context-token", "context-token-secret")
	if err := runIssueCreate(cmd, nil); err != nil {
		t.Fatalf("runIssueCreate: %v", err)
	}
	if got := body["agent_identity_context_token"]; got != "context-token-secret" {
		t.Fatalf("agent_identity_context_token = %#v", got)
	}
}
