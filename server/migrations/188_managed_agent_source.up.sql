...
-- GitHub Agents, but their source is one deployment-owned public Git repo.
-- The compiled bundle is a PostgreSQL last-known-good snapshot so request
-- handling never depends on a node-local checkout or live Gitee availability.

CREATE TABLE managed_agent_source_snapshot (
...
ALTER TABLE agent_source
    ...
    ADD COLUMN managed_source_key TEXT REFERENCES managed_agent_source_snapshot(source_key) ON DELETE RESTRICT;
...
ALTER TABLE agent_source
    ...
    ADD CONSTRAINT agent_source_source_mode_check CHECK (
        ...
        OR
        (source_type = 'managed_git' AND github_installation_id IS NULL AND managed_source_key IS NOT NULL)
    );

CREATE UNIQUE INDEX agent_source_workspace_managed_key_idx
